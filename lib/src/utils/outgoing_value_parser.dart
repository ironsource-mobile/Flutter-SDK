...
class OutgoingValueParser {
  ...
  /// Maps [appKey] and [adFormats] to the keys and returns the [Map].
  /// ```
  /// { 'appKey': String, 'adFormats': List<String>? }
  /// ```
  static Map<String, dynamic> initLevelPlay({required String appKey, List<AdFormat>? adFormats, String? userId}) {
    final Map<String, dynamic> args = {IronConstKey.APP_KEY: appKey, IronConstKey.USER_ID: userId};
    if (adFormats != null && adFormats.isNotEmpty) {
      args[IronConstKey.AD_FORMATS] =
          adFormats.map((unit) => unit.value).toList();
    }
    return args;
  }
